...
function generateRemark(index, port, address, cleanIPs, protocol, configType) {
    ...
    return `BENIYAMIN ${index} - ${protocol}${type} - ${addressType} : ${port}`;
}
...
async function renderHomePage (env, hostName, fragConfigs) {
	...
	<head>
        ...
        <title>BENIYAMIN ${panelVersion}</title>
	...
	<body>
		<h1>BENIYAMIN <span style="font-size: smaller;">${panelVersion}</span> BENIYAMIN</h1>
    ...
    warpEndpoints.split(',').forEach( (endpoint, index) => {
        ...
        if (client === 'singbox' || client === 'hiddify') {
            let singboxOutbound = buildSingboxWarpOutbound(
                client === 'hiddify' ? `BENIYAMIN Warp Pro ${index + 1} 🇮🇷` : `BENIYAMIN Warp ${index + 1} 🇮🇷`, 
                warpIPv6, 
                privateKey, 
                publicKey, 
                endpoint,
        ...
        if (client === 'clash') {
            let clashOutbound = buildClashWarpOutbound(`BENIYAMIN Warp ${index + 1} 🇮🇷`, warpIPv6, privateKey, publicKey, endpoint, reserved, '');
        ...
        for (let i = 0; i < 2; i++) {
            ...
            if (client === 'singbox' || client === 'hiddify') {
                let singboxOutbound = buildSingboxWarpOutbound(
                    ...
                    ? `warp-ir_${index + 1}` 
                    : client === 'hiddify' 
                        ? `BENIYAMIN WoW Pro ${index + 1} 🌍` 
                        : `BENIYAMIN WoW ${index + 1} 🌍` , 
                    warpIPv6, 
                    privateKey, 
                    publicKey, 
                    endpoint, 
            ...
            if (client === 'clash') {
                let clashOutbound = buildClashWarpOutbound(
                    i === 1 ? `warp-ir_${index + 1}` : `BENIYAMIN WoW ${index + 1} 🌍`, 
                    warpIPv6, 
                    privateKey, 
                    publicKey, 
                    endpoint,
...
async function buildWorkerLessConfig(remoteDNS, localDNS, lengthMin,  lengthMax,  intervalMin,  intervalMax, blockAds, bypassIran, blockPorn, bypassLAN, bypassChina, blockUDP443) {
    ...
    fragConfig.remarks  = 'BENIYAMIN F - -YouTube(یوتیوب) 📺-'
...
async function getFragmentConfigs(env, hostName) {
    ...
    bestPing.remarks = 'BENIYAMIN -بهترین پینگ 🏎️-';
    ...
    bestFragment.remarks = 'BENIYAMIN -فرگمنت خودکار 🔥-';
...
async function getXrayWarpConfigs (env, client) {
    ...
    xrayWarpBestPing.remarks = client === 'nikang' ? 'BENIYAMIN -بهترین پینگ 🏎️-' : 'BENIYAMIN -بهترین پینگ 🏎️-';
    ...
    xrayWarpOutbounds.forEach((outbound, index) => {
        xrayWarpConfigs.push({
            ...xrayWarpConfig,
            remarks: client === 'nikang' ? `BENIYAMIN Warp Pro ${index + 1} 🇮🇷` : `BENIYAMIN Warp ${index + 1} 🇮🇷`,
    ...
    xrayWoWOutbounds.forEach((outbound, index) => {
        if (outbound.tag.includes('warp-out')) {
            ...
            xrayWoWConfig.remarks = client === 'nikang' ? `BENIYAMIN WoW Pro ${index/2 + 1} 🌍` : `BENIYAMIN WoW ${index/2 + 1} 🌍`;
    ...
    xrayWoWBestPing.remarks = client === 'nikang' ? 'BENIYAMIN -بهترین پینگ 🏎️-' : 'BENIYAMIN -بهترین پینگ 🏎️-';
...
function buildClashChainOutbound(chainProxyParams) {
    ...
    let chainOutbound = {
        ...
        "flow": flow,
        "network": type,
        "dialer-proxy": "BENIYAMIN Best Ping -بهترین پینگ 🏎️-"
    };
...
async function getClashConfig (env, hostName, isWarp) {
    ...
    config['proxy-groups'][0].proxies = isWarp
        ? ['BENIYAMIN -بهترین پینگ 🏎️-', 'BENIYAMIN -بهترین پینگ 🏎️-', ...warpOutboundsRemarks, ...wowOutboundRemarks ]
        : ['BENIYAMIN -بهترین پینگ 🏎️-', ...outboundsRemarks ];

    config['proxy-groups'][1].proxies = isWarp ? warpOutboundsRemarks : outboundsRemarks;
    config['proxy-groups'][1].name = isWarp ? `BENIYAMIN -بهترین پینگ 🏎️-`: `BENIYAMIN -بهترین پینگ 🏎️-`,
...
async function getNormalConfigs(env, hostName, client) {
    ...
    if (outProxy) {
        let chainRemark = `#${encodeURIComponent('BENIYAMIN Chain proxy 🔗')}`;
        ...
        servers: [
            {
                ...
                detour: "BENIYAMIN -بهترین پینگ 🏎️-",
                tag: "dns-remote"
            },
            {
                address: "",
    ...
    outbounds: [
        {
            type: "selector",
            tag: "proxy",
            outbounds: ["BENIYAMIN -بهترین پینگ 🏎️-"]
        },
        {
            type: "urltest",
            tag: "BENIYAMIN -بهترین پینگ 🏎️-",
            outbounds: [],
            url: "https://www.gstatic.com/generate_204",
            interval: "30s",
            tolerance: 50
