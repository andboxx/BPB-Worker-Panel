...
function generateRemark(index, port, address, cleanIPs, protocol, configType) {
    ...
    return `KAMAL ${index} - ${protocol}${type} - ${addressType} : ${port}`;
}
...
async function renderHomePage (env, hostName, fragConfigs) {
	...
	<head>
        ...
        <title>KAMAL ${panelVersion}</title>
	...
	<body>
		<h1>KAMAL <span style="font-size: smaller;">${panelVersion}</span> KAMAL</h1>
    ...
    warpEndpoints.split(',').forEach( (endpoint, index) => {
        ...
        if (client === 'singbox' || client === 'hiddify') {
            let singboxOutbound = buildSingboxWarpOutbound(
                client === 'hiddify' ? `KAMAL Warp Pro ${index + 1} 🇮🇷` : `KAMAL Warp ${index + 1} 🇮🇷`, 
                warpIPv6, 
                privateKey, 
                publicKey, 
                endpoint,
        ...
        if (client === 'clash') {
            let clashOutbound = buildClashWarpOutbound(`KAMAL Warp ${index + 1} 🇮🇷`, warpIPv6, privateKey, publicKey, endpoint, reserved, '');
        ...
        for (let i = 0; i < 2; i++) {
            ...
            if (client === 'clash') {
                let clashOutbound = buildClashWarpOutbound(
                    i === 1 ? `warp-ir_${index + 1}` : `KAMAL WoW ${index + 1} 🌍`, 
                    warpIPv6, 
                    privateKey, 
                    publicKey, 
                    endpoint,
...
async function buildWorkerLessConfig(remoteDNS, localDNS, lengthMin,  lengthMax,  intervalMin,  intervalMax, blockAds, bypassIran, blockPorn, bypassLAN, bypassChina, blockUDP443) {
    ...
    fragConfig.remarks  = 'KAMAL F - -YouTube(یوتیوب) 📺-'
...
async function getFragmentConfigs(env, hostName) {
    ...
    bestPing.remarks = 'KAMAL -بهترین پینگ 🏎️-';
    ...
    bestFragment.remarks = 'KAMAL -فرگمنت خودکار 🔥-';
...
async function getXrayWarpConfigs (env, client) {
    ...
    xrayWarpBestPing.remarks = client === 'nikang' ? 'KAMAL -بهترین پینگ 🏎️-' : 'KAMAL -بهترین پینگ 🏎️-';
    ...
    xrayWarpOutbounds.forEach((outbound, index) => {
        xrayWarpConfigs.push({
            ...xrayWarpConfig,
            remarks: client === 'nikang' ? `KAMAL Warp Pro ${index + 1} 🇮🇷` : `KAMAL Warp ${index + 1} 🇮🇷`,
    ...
    xrayWoWOutbounds.forEach((outbound, index) => {
        if (outbound.tag.includes('warp-out')) {
            ...
            xrayWoWConfig.remarks = client === 'nikang' ? `KAMAL WoW Pro ${index/2 + 1} 🌍` : `KAMAL WoW ${index/2 + 1} 🌍`;
    ...
    xrayWoWBestPing.remarks = client === 'nikang' ? 'KAMAL -بهترین پینگ 🏎️-' : 'KAMAL -بهترین پینگ 🏎️-';
...
function buildClashChainOutbound(chainProxyParams) {
    ...
    let chainOutbound = {
        ...
        "flow": flow,
        "network": type,
        "dialer-proxy": "KAMAL Best Ping -بهترین پینگ 🏎️-"
    };
...
async function getClashConfig (env, hostName, isWarp) {
    ...
    config['proxy-groups'][0].proxies = isWarp
        ? ['KAMAL -بهترین پینگ 🏎️-', 'KAMAL -بهترین پینگ 🏎️-', ...warpOutboundsRemarks, ...wowOutboundRemarks ]
        : ['KAMAL -بهترین پینگ 🏎️-', ...outboundsRemarks ];

    config['proxy-groups'][1].proxies = isWarp ? warpOutboundsRemarks : outboundsRemarks;
    config['proxy-groups'][1].name = isWarp ? `KAMAL -بهترین پینگ 🏎️-`: `KAMAL -بهترین پینگ 🏎️-`,
...
async function getSingboxConfig (env, hostName, client, isWarp, isFragment) {
    ...
    if (isWarp) {
        ...
        config.outbounds[0].outbounds = client === 'hiddify'
            ? ["KAMAL -بهترین پینگ 🏎️-", "KAMAL -بهترین پینگ 🏎️-"]
            : ["KAMAL -بهترین پینگ 🏎️-", "KAMAL -بهترین پینگ 🏎️-"];
...
async function getNormalConfigs(env, hostName, client) {
    ...
    if (outProxy) {
        let chainRemark = `#${encodeURIComponent('KAMAL Chain proxy 🔗')}`;
    ...
    outbounds: [
        {
            type: "selector",
            tag: "proxy",
            outbounds: ["KAMAL -بهترین پینگ 🏎️-"]
        },
        {
            type: "urltest",
            tag: "KAMAL -بهترین پینگ 🏎️-",
            outbounds: [],
            url: "https://www.gstatic.com/generate_204",
            interval: "30s",
            tolerance: 50
