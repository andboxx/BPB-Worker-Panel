...
function generateRemark(index, port, address, cleanIPs, protocol, configType) {
    ...
    return `X-کمال-X ${index} - ${protocol}${type} - ${addressType} : ${port}`;
}
...
async function renderHomePage (env, hostName, fragConfigs) {
	...
	<head>
        ...
        <title>X-کمال-X ${panelVersion}</title>
	...
	<body>
		<h1>X-کمال-X <span style="font-size: smaller;">${panelVersion}</span> 🆒</h1>
    ...
    warpEndpoints.split(',').forEach( (endpoint, index) => {
        ...
        if (client === 'singbox' || client === 'hiddify') {
            let singboxOutbound = buildSingboxWarpOutbound(
                client === 'hiddify' ? `X-کمال-X Warp Pro ${index + 1} 🦁☀️` : `X-کمال-X Warp ${index + 1} 🦁☀️`, 
                warpIPv6, 
                privateKey, 
                publicKey, 
                endpoint,
        ...
        if (client === 'clash') {
            let clashOutbound = buildClashWarpOutbound(`X-کمال-X Warp ${index + 1} 🦁☀️`, warpIPv6, privateKey, publicKey, endpoint, reserved, '');
        ...
        for (let i = 0; i < 2; i++) {
            ...
            if (client === 'singbox' || client === 'hiddify') {
                let singboxOutbound = buildSingboxWarpOutbound(
                    i === 1
                        ? `proxy-${index + 1}` 
                        : client === 'hiddify' 
                            ? `X-کمال-X WoW Pro ${index + 1} 🌐` 
                            : `X-کمال-X WoW ${index + 1} 🌐` , 
                    warpIPv6, 
                    privateKey, 
                    publicKey, 
                    endpoint, 
            ...
            if (client === 'clash') {
                let clashOutbound = buildClashWarpOutbound(
                    i === 1 ? `proxy-${index + 1}` : `X-کمال-X WoW ${index + 1} 🌐`, 
                    warpIPv6, 
                    privateKey, 
                    publicKey, 
                    endpoint,
...
async function buildXrayWorkerLessConfig(remoteDNS, localDNS, lengthMin,  lengthMax,  intervalMin,  intervalMax, fragmentPackets, blockAds, bypassIran, blockPorn, bypassLAN, bypassChina, blockUDP443) {
    ...
    fragConfig.remarks  = 'X-کمال-X  - YouTube(یوتیوب) 📺'
...
async function getXrayFragmentConfigs(env, hostName) {
    ...
    bestPing.remarks = 'X-کمال-X  - بهترین پینگ 🏎️';
    ...
    bestFragment.remarks = 'X-کمال-X  - فرگمنت خودکار 🔥';
...
async function getXrayWarpConfigs (env, client) {
    ...
    xrayWarpBestPing.remarks = client === 'nikang' ? 'X-کمال-X وارپ پرو بهترین پینگ 🏎️' : 'X-کمال-X وارپ بهترین پینگ 🏎️';
    ...
    xrayWarpOutbounds.forEach((outbound, index) => {
        xrayWarpConfigs.push({
            ...xrayWarpConfig,
            remarks: client === 'nikang' ? `X-کمال-X Warp Pro ${index + 1} 🦁☀️` : `X-کمال-X Warp ${index + 1} 🦁☀️`,
    ...
    xrayWoWOutbounds.forEach((outbound, index) => {
        if (outbound.tag === 'chain') {
            ...
            xrayWoWConfig.remarks = client === 'nikang' ? `X-کمال-X WoW Pro ${proxyIndex} 🌐` : `X-کمال-X WoW ${proxyIndex} 🌐`;
    ...
    xrayWoWBestPing.remarks = client === 'nikang' ? 'X-کمال-X وارپ آن وارپ پرو بهترین پینگ 🏎️' : 'X-کمال-X وارپ آن وارپ بهترین پینگ 🏎️';
...
function buildClashChainOutbound(chainProxyParams) {
    ...
    const { hostName, port, uuid, flow, security, type, sni, fp, alpn, pbk, sid, spx, headerType, host, path, authority, serviceName, mode } = chainProxyParams;
    let chainOutbound = {
        "name": "X-کمال-X بهترین پینگ زنجیره🏎️",
        "type": "vless",
        "server": hostName,
        "port": +port,
        "udp": true,
        ...
        "flow": flow,
        "network": type,
        "dialer-proxy": "X-کمال-X بهترین پینگ 🏎️"
    };
...
async function getClashConfig (env, hostName, isWarp) {
    ...
    if (isWarp) {
        config['proxy-groups'][0].proxies = ['X-کمال-X وارپ بهترین پینگ 🏎️', 'X-کمال-X وارپ آن وارپ بهترین پینگ 🏎️'];
        config['proxy-groups'][1].name = 'X-کمال-X وارپ بهترین پینگ 🏎️';
        config['proxy-groups'][1].interval = +bestWarpInterval;
        config['proxy-groups'].splice(2, 0, structuredClone(config['proxy-groups'][1]));
        config['proxy-groups'][2].name = 'X-کمال-X وارپ آن وارپ بهترین پینگ 🏎️';
        ...
        config['proxy-groups'][0].proxies = ['X-کمال-X بهترین پینگ 🏎️'];
        config['proxy-groups'][1].name = 'X-کمال-X بهترین پینگ 🏎️';
...
async function getSingboxConfig (env, hostName, client, isWarp, isFragment) {
    ...
    if (isWarp) {
        ...
        config.outbounds[0].outbounds = client === 'hiddify'
            ? ["X-کمال-X وارپ پرو بهترین پینگ 🏎️", "X-کمال-X وارپ آن وارپ پرو بهترین پینگ 🏎️"]
            : ["X-کمال-X وارپ بهترین پینگ 🏎️", "X-کمال-X وارپ آن وارپ بهترین پینگ 🏎️"];
        config.outbounds.splice(2, 0, structuredClone(config.outbounds[1]));
        config.outbounds[1].tag = client === 'hiddify' 
            ? "X-کمال-X وارپ پرو بهترین پینگ 🏎️"
            : "X-کمال-X وارپ بهترین پینگ 🏎️";
        config.outbounds[2].tag = client === 'hiddify'
            ? "X-کمال-X وارپ آن وارپ پرو بهترین پینگ 🏎️"
            : "X-کمال-X وارپ آن وارپ بهترین پینگ 🏎️";
...
async function getNormalConfigs(env, hostName, client) {
    ...
    if (outProxy) {
        let chainRemark = `#${encodeURIComponent('X-کمال-X Chain proxy 🔗')}`;
    ...
    outbounds: [
        {
            type: "selector",
            tag: "proxy",
            outbounds: ["X-کمال-X بهترین پینگ 🏎️"]
        },
        {
            type: "urltest",
            tag: "X-کمال-X بهترین پینگ 🏎️",
            outbounds: [],
